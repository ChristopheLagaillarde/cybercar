...
public class QueryStatement {
	...
	"INSERT INTO employe(nom, prenom, adresse, login, motDePasse, departement, civilite, nomDeJeuneFille, telephone, mail, situationConjugale, nombreEnfants, entecedantMedicale, fonction, dateEmbauche, salaire, nombreConge, nombreCongeRestant, dateFinContrat) VALUES(?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?)";

	public final static String
	UPDATE_EMPLOYE_QUERY =

	"UPDATE employe SET nom=?, prenom=?, adresse=?, login=?, motDePasse=?, departement=?, civilite=?, nomDeJeuneFille=?, telephone=?, e-mail=?, situationConjugale=?, nombreEnfants=?, entecedantMedicale=?, fonction=?, dateEmbauche=?, salaire=?, nombreConge=?, nombreCongeRestant=?, dateFinContrat=? where id_employé= ?";

	public final static String DELETE_EMPLOYE_QUERY =

	"DELETE FROM employe WHERE id_employé=?";
